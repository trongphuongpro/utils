...
def createEntries(directoryName, category):
	...
	# don't create dir bin for library project
	if category[:3] != "lib":
		os.mkdir('bin')
	...
	# copy CMakeList.txt template to project for testing
	# ex. if category is libavr, then copy file avr-CMakeLists.txt for testing
	if category[:3] == "lib":
		templateFile = templatesPath + cmakeTemplatesDir + category[3:] + "-CMakeLists.txt"
